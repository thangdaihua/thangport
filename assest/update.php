    ...
    if (isset($_POST["update"])) {
        ...
        switch ($type) {
            case "article":
                ...
                $short = test_input($_POST["arShort"]);
                $content = $_POST["arContent"];
                $categorie = test_input($_POST["arCategory"]);
                $author = test_input($_POST["arAuthor"]);
                $imageName = test_input($_POST["arImage"]);

              

                try {
                    $sql = "UPDATE `article`
                        SET `article_title`= ?, `article_content`= ?,`article_image`=?, `id_categorie`=?, `id_author`= ? ,`article_short`= ? 
                        WHERE `article_id` = ?";

                    $stmt = $conn->prepare($sql);

                    $stmt->execute([$title, $content, $imageName, $categorie, $author, $short, $urlId]);
                ...
                // Go to show.php
                // header("refresh:1; url=../qnvvpzmzgkadminadmin.php");
                header("Location: ../qnvvpzmzgkadminadmin.php", true, 301);
                exit;
                break;
            ...
            case "category":
                ...
                // Go to show.php
                // header("refresh:1; url=../allqnvvpzmzgkcategories.php");
                header("Location: ../qnvvpzmzgkcategories.php", true, 301);
                exit;
                ...
                case "uishot":
                    ...
                    // Update DataBase
                    $title = test_input($_POST["uiShotTitle"]);
                    $image = test_input($_POST["uiShotImg"]);
                    $high = test_input($_POST["uiShotHigh"]);
                    
                    try {
                        $sql = "UPDATE `uishot`
                            SET `uishot_image`= ?, `uishot_title`= ?,`uishot_high`=?
                            WHERE `uishot_id` = ?";
    
                        $stmt = $conn->prepare($sql);
    
                        $stmt->execute([$image, $title, $high, $urlId]);
                    ...
                    // Go to show.php
                    // header("refresh:1; url=../allqnvvpzmzgkcategories.php");
                    header("Location: ../iyvjqamvceuishots.php", true, 301);
                    exit;
